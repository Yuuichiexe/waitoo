...
from Hori.modules.disable import MessageHandler

OWO = (
    "*Horix pats {} on the head.",
    "*gently rubs {}'s head*.",
    "*Horix mofumofus {}'s head*",
    "*Horix messes up {}'s head*",
    "*Horix intensly rubs {}'s head*",
...
def kiss(update, context):
    url = "https://Horis.best/api/v2/kiss"
    r = requests.get(url)
    e = r.json()
...
def pat(update, context):
    ...
    url = "https://Horis.best/api/v2/pat"
    r = requests.get(url)
    e = r.json()
...
def hug(update, context):
    msg = update.effective_message
    url = "https://Horis.best/api/v2/hug"
    r = requests.get(url)
    e = r.json()
...
def slap(update, context):
    msg = update.effective_message
    url = "https://Horis.best/api/v2/slap"
    r = requests.get(url)
    e = r.json()
...
def blush(update, context):
    msg = update.effective_message
    url = "https://Horis.best/api/v2/blush"
    r = requests.get(url)
    e = r.json()
...
def cute(update, context):
    ...
    url = "https://Horis.best/api/v2/Horix"
    r = requests.get(url)
    e = r.json()
...
def sleep(update, context):
    ...
    if sleep_type == "Gif":
        msg = update.effective_message
        url = "https://Horis.best/api/v2/sleep"
        r = requests.get(url)
        e = r.json()
...
KISS_HANDLER = MessageHandler(Filters.regex("(?i)Horix kiss"), kiss, run_async=True)
BLUSH_HANDLER = MessageHandler(Filters.regex("(?i)Horix blush"), blush, run_async=True)
PAT_HANDLER = MessageHandler(Filters.regex("(?i)Horix pat"), pat, run_async=True)
HUG_HANDLER = MessageHandler(Filters.regex("(?i)Horix hug"), hug, run_async=True)
SLAP_HANDLER = MessageHandler(Filters.regex("(?i)Horix slap"), slap, run_async=True)
CUTE_HANDLER = MessageHandler(Filters.regex("(?i)Horix cute"), cute, run_async=True)
SLEEP_HANDLER = MessageHandler(
    Filters.regex("(Horix sleep|sleep)"), sleep, run_async=True
)
